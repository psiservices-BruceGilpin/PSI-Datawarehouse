...
from 
  Candidate.TestSchedule a with (nolock) 
  join Candidate.TestAssignments b with (nolock) on 
	a.CandidateTestScheduleDBID = b.TestScheduleKey 
	and b.CurrentFlag = 0 
  join candidate.candidates c with (nolock) on 
	b.candidatekey = c.candidatedbid 
	and c.CurrentFlag = 0 
  join Candidate.TestAssignmentPortions d with (nolock) on 
	b.TestAssignmentDBID = d.TestAssignKey 
	and d.CurrentFlag = 0 
  join test.ItemPool e with (nolock) on 
	d.TestPortionKey = e.PortionKey 
	and e.CurentFlag = 0 
  right join test.ItemBank f with (nolock) on 
	e.itemkey = f.ItemBankDBID 
	and f.currentflag = 0 
  join test.tests h with (nolock) on 
	h.TestDBKey = b.TestKey 
	and h.currentflag = 0 
  join Account.Accounts i with (nolock) on 
	h.AccountKey = i.AccountDBID 
	and i.CurrentFlag = 0 
  left join Candidate.Responses j with (nolock) on 
	j.TestAssignPortionKey = d.TestAssignPortionDBId 
	and j.ItemBankKey = f.ItemBankDBID 
	and j.CurrentFlag = 0 
  left join test.FixedForm k with (nolock) on 
	j.FixedFormKey = k.FixedFormDBID 
	and k.CurrentFlag = 0 join
  test.Portion l with (nolock) on
	d.TestPortionKey = l.PortionDBID 
  join SourceSystems m with (nolock) on
